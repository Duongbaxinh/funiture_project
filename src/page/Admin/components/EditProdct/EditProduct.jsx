...
function EditProduct() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setProductUpdate(data.metadata)
        }
        try {
            fetchData()
        } catch (error) {
    ...
    const handleChangeInforProduct = (e, filed) => {
        ...
        console.log(cloneProductUpdata.categoryId)
        setProductUpdate({ ...cloneProductUpdata })
    }
    if (!productUpdate) { return <h1>Loading...</h1> }
    return (
        <>
            <div className='editporoduct'>
                <Sidebar />
                <div className="editproduct_content" style={{ width: '100%', marginLeft: '350px', padding: '30px' }}>
